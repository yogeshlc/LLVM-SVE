...
void DAGTypeLegalizer::PromoteIntegerResult(SDNode *N, unsigned ResNo) {
  ...
  case ISD::TEST_VECTOR: Res = PromoteIntRes_TEST_VECTOR(N); break;
  case ISD::ELEMENT_COUNT:
                         Res = PromoteIntRes_ELEMENT_COUNT(N); break;
...
SDValue DAGTypeLegalizer::PromoteIntRes_MGATHER(MaskedGatherSDNode *N) {
  ...
  SDValue Ops[] = { N->getChain(), ExtSrc0, Mask, N->getBasePtr(),
                    N->getIndex() };
...
bool DAGTypeLegalizer::PromoteIntegerOperand(SDNode *N, unsigned OpNo) {
  ...
  case ISD::SERIES_VECTOR:
                          Res = PromoteInt_SERIES_VECTOR(N); break;
  case ISD::ELEMENT_COUNT:
                          Res = PromoteInt_ELEMENT_COUNT(N); break;
  case ISD::VSELECT:
...
SDValue DAGTypeLegalizer::PromoteIntOp_BITCAST(SDNode *N) {
  assert(!N->getValueType(0).isScalableVector() &&
         "Can't store-load scalable vector");
...
SDValue DAGTypeLegalizer::PromoteIntOp_MSTORE(MaskedStoreSDNode *N,
                                              unsigned OpNo){
  assert((OpNo != 1) && "Not expecting to promote the base pointer!");
...
SDValue DAGTypeLegalizer::PromoteIntOp_MSCATTER(MaskedScatterSDNode *N,
  ...
  return DAG.getMaskedScatter(DAG.getVTList(MVT::Other), N->getMemoryVT(), SDLoc(N),
                              NewOps, N->getMemOperand(), TruncateStore);
}
...
SDValue DAGTypeLegalizer::PromoteInt_ELEMENT_COUNT(SDNode *N) {
  SDLoc dl(N);
  ...
  assert(OpVT.getVectorNumElements() == WideVT.getVectorNumElements());
  return DAG.getNode(ISD::ELEMENT_COUNT, dl, VT, DAG.getValueType(WideVT));
}
...
SDValue DAGTypeLegalizer::PromoteIntRes_ELEMENT_COUNT(SDNode *N) {
  // Extend the original operand all the way up to the larger type.
  EVT NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  return DAG.getNode(N->getOpcode(), SDLoc(N), NVT, N->getOperand(0));
}

SDValue DAGTypeLegalizer::PromoteIntRes_TEST_VECTOR(SDNode *N) {
  // Extend the original operand all the way up to the larger type.
  EVT NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  return DAG.getNode(N->getOpcode(), SDLoc(N), NVT, N->getOperand(0), N->getOperand(1));
}
