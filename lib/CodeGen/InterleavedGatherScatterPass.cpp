...
private:
  ...
  struct StrideDescriptor {
    ...
    const SCEV *BaseSCEV;
    // The vector of offsets from the base ptr.
    SeriesVectorInst *Index;
    ...
    static std::unique_ptr<StrideDescriptor> create(IntrinsicInst *Instr,
                                                    ScalarEvolution *SE,
                                                    const DataLayout *DL) {
      Value *Base, *Align, *GP, *Merge;
      Type *CastDestTy = nullptr;
      bool IsLoad;
      if (match(Instr, m_Intrinsic<Intrinsic::masked_gather>(
                m_Value(Base), m_Value(Align), m_Value(GP), m_Value(Merge))))
        IsLoad = true;
      ...
      auto Index = dyn_cast<SeriesVectorInst>(GEP->getOperand(1));
      if (!Index)
        return nullptr;
      auto StrideInst = dyn_cast<ConstantInt>(Index->getOperand(1));
      if (!StrideInst)
        return nullptr;
      ...
      return llvm::make_unique<StrideDescriptor>(
          Instr, BaseSCEV, Index, GP, Stride, Size, EltTy,
          ConstantAlign->getSExtValue(), CastDestTy);
    }
    StrideDescriptor(IntrinsicInst *Instruction, const SCEV *BaseSCEV,
                     SeriesVectorInst *Index, Value *GP, int Stride,
                     unsigned Size, Type *EltTy, unsigned Align,
                     Type *CastDestTy)
        : Instruction(Instruction), BaseSCEV(BaseSCEV), Index(Index), GP(GP),
...
INITIALIZE_PASS_BEGIN(
  InterleavedGatherScatter, "interleaved-gather-scatter", ia_name, false, false)
...
FunctionPass *llvm::createInterleavedGatherScatterPass(const TargetMachine *TM){
...
bool InterleavedGatherScatter::lowerSDGroup(StrideGroup &Group) {
  ...
  if (IsLoad)
    ...
    Changed = TLI->lowerScattersToInterleavedStore(Values,
                                           LowestSD->Instruction->getOperand(1),
                                           LastSD->Instruction, Factor, TTI);
...
bool InterleavedGatherScatter::addSDToGroup(StrideDescriptor &SD,
  ...
  assert(SD0->Instruction->getIntrinsicID() == SD.Instruction->getIntrinsicID()
  ...
  if ((SD0->Index != SD.Index) || (SD0->Stride != SD.Stride) ||
...
bool InterleavedGatherScatter::lowerGatherScatterNoAliasGroup(
  ...
  for(auto Intrinsic : NoAliasGroup) {
    ...
    if (SD) {
      ...
      DEBUG(dbgs() << "IGS: Skipping (failed to Create SD): "
            << *Intrinsic << "\n");
      continue;
    }
    ...
    if (SD->Stride < 0) {
      ...
      DEBUG(dbgs() << "IGS: Skipping (negative stride): "
            << *SD->Instruction << "\n");
      continue;
    }
...
bool InterleavedGatherScatter::sinkScatterStores(BasicBlock *BB,
  ...
  for (auto It = GSInBlock.begin(), E = GSInBlock.end(); It != E; ++It) {
    ...
    if (!I->mayWriteToMemory())
      continue;
    DEBUG(dbgs() << "Re-order: Trying to sink store: " << *I << "\n");

    if (!BasePtr) {
      DEBUG(dbgs() << "Re-order: No base ptr found, giving up.\n");
      continue;
    }

    // Check whether the scatter is currently adjacent to another scatter,
    // if so don't try to sink it as we could end up breaking a potential
    // stride group later.
    auto PrevGS = It;
    if (It != GSInBlock.begin())
      --PrevGS;
    auto NextGS = It;
    if (std::next(It) != GSInBlock.end())
      ++NextGS;
    if ((PrevGS != It && PrevGS->first->mayWriteToMemory()) ||
        (NextGS != It && NextGS->first->mayWriteToMemory()))
      continue;
    ...
    for (auto End = I->getParent()->end(); Next != End; ++Next) {
      ...
      if (auto LI = dyn_cast<LoadInst>(NextI)) {
        if (!LI->isSimple())
          break; // Can't sink past atomic or volatile loads.
        ...
        if (!NextPtr)
          break; // The base pointer couldn't be found so we have to stop.
    ...
    DEBUG(dbgs() << "Re-order: Sunk scatter instruction.\n");
    Changed = true;
  }
...
Value *InterleavedGatherScatter::findBasePtrFromVector(Value *VecPtr) {
  ...
  if (!isa<SeriesVectorInst>(BaseGEP->getOperand(1)))
    return nullptr;
