...
public:
  ...
  bool SelectAddrModeIndexedSVE(SDValue N, SDValue &Base, SDValue &OffImm);
...
/// SelectAddrModeIndexedSVE - Select a "register plus scaled signed 9-bit
/// immediate" address.  The scale is determined at runtime by the size of a
/// SVE register on this particular hardware using the 'rdvl' instruction
template <int64_t Min, int64_t Max>
bool AArch64DAGToDAGISel::SelectAddrModeIndexedSVE(SDValue N, SDValue &Base,
                                                   SDValue &OffImm) {
  if (N->getOpcode() == ISD::FrameIndex) {
    SDLoc dl(N);
    const DataLayout &DL = CurDAG->getDataLayout();
  ...
  // The machinery provided in this method is a working proof of concept that
  // will be extracted to a class that will make all this transformations
  // available in DAGCombine.

  // The algorithm works by moving any expression involving the composition of
  // ELEMENT_COUNT with constant values into the RHS (or LHS) of the node N used
  // as base address of the load/store, in the following form:
  //
  //       (BASE + ELEMENT_COUNT * K),                                    (1)
  //
  // where:
  // * BASE is an arbitrary expression not involving constant computations of
  //   element count.
  // * K is a constant number.
  //
  // This reduction is performed so that a LOAD from the expression (1) can be
  // generated as a SVE reg+immediate LOAD as:
  //
  //        ldr z0, [BASE, #k, mul vl]
  //
  // The tests show the following tranformations:
  //
  // ### CASE 1:
  // (add base1,
  //      (add base2, ec))
  ...
  //
  // ### CASE 5:
  // Notice that EC expression is mixed with base in the LHS of teh outermost
  // add and thet the whole expression is shifted by a constant which need to be
  // threated as a base.
  //
  ...
  // Typedef for fractions.
  // Ring operations are provided: add, mul.
  // Additional operations like shl, srl are derived from the ring structure
  // ones.
  // The type is marked as 'const' to avoid non explicit initialization,
  // variable assignments and in place modification.
  using fraction = const std::pair<int64_t, int64_t>;

  // List of ISD::ELEMENT_COUNT nodes found in the node subgraph.
  SmallVector<const SDNode *, 2> FoundEC;

  // Vector of base addresses used to form the final base address. The boolean
  // value is to determine if the node need to be added to the expression or
  // subtracted.
  SmallVector<std::pair<SDNode *, bool>, 2> BaseComponent;

  // Check if a SDValue is a base
  const std::function<bool(const SDValue &)> isBase = [&isBase](const SDValue &
                                                                S)->bool {

    const SDNode *N = S.getNode();
    unsigned Opcode = N->getOpcode();
    switch (Opcode) {
    case ISD::ADD:
    case ISD::SUB: {
      SDValue LHS = S.getOperand(0);
      SDValue RHS = S.getOperand(1);
      // ADD and SUB nodes are bases if all their operands are
      return isBase(LHS) && isBase(RHS);
    }
    case ISD::SHL:
    case ISD::SRL:
      // shifts are bases if the LHS is a base.
      return isBase(S.getOperand(0));
    case ISD::MUL: {
      // MULS are bases in general, but if one of the operand is a constant we
      // just need to check the other operand. This essentially isolate the EC
      // multiple from the concept of being a base.
      SDValue LHS = S.getOperand(0);
      SDValue RHS = S.getOperand(1);
    ...
    // Constant are always bases
    case ISD::Constant:
      return true;
    case ISD::ELEMENT_COUNT:
      // EC's are not bases themselves
      return false;
    default:
      // Be conservative on unknown nodes
      return true;
    }
    llvm_unreachable("returns must stay in switch statement");
  };

  // Recursive function for offset computation.
  const std::function<fraction(const SDValue &, bool Negate)> rec = [
    &rec,  &isBase, &BaseComponent, &FoundEC, this
  ](const SDValue &S, bool Negate)->fraction {

  const auto mul = [](fraction &A, fraction &B)->fraction {
    assert((A.second != 0) && "Invalid fraction");
    assert((B.second != 0) && "Invalid fraction");
    return { A.first * B.first, A.second * B.second };
  };

#ifndef NDEBUG
    EVT VT = S.getValueType();
    MVT Ty = VT.getSimpleVT();
    assert((Ty == MVT::i64) && "not a valid type");
#endif

    // Is this value a base component? This call ensures that the recursive
    // computation of the OFFSET value is triggered only for expression which
    // are reducible to constant multipliers of an ELEMENT_COUNT (in
    // mathematical sense). 
    if (isBase(S)) {
      DEBUG_WITH_TYPE("am_indexed_sve",
                      dbgs() << "\n*** SelectAddrModeIndexedSVE ***\n";
            dbgs() << "Machine function: "
                   << CurDAG->getMachineFunction().getName() << "\n";
            dbgs() << "Adding BASE component:\n"; S.getNode()->dumpr());
      BaseComponent.emplace_back(S.getNode(), Negate);
      return { 0, 1 };
    }

    const unsigned Opcode = S.getOpcode();

    // An element_count is counted as a unity. At this level, the EC is not
    // mixed with bases because bases have been filtered out in the previous if
    // statement.
    if (Opcode == ISD::ELEMENT_COUNT) {
      assert(cast<VTSDNode>(S.getOperand(0))->getVT() == MVT::nxv16i8 &&
             "Expected vector type with a directly mapping to RDVL.");
      FoundEC.push_back(S.getNode());
      return { 1, 1 };
    }

    // At this level all operations are linear on EC and involve only EC's and
    // constants.
    assert((S.getNumOperands() == 2) && "node must be a binary operator");
    SDValue LHSVal = S.getOperand(0);
    SDValue RHSVal = S.getOperand(1);
    switch (Opcode) {
    case ISD::ADD: {
      auto LHS = rec(LHSVal, false);
      auto RHS = rec(RHSVal, false);
    ...
    case ISD::SUB: {
      auto LHS = rec(LHSVal, false);
      auto RHS = rec(RHSVal, true);
      // subtraction of fractions
      return { LHS.first * RHS.second - RHS.first * LHS.second,
          LHS.second * RHS.second };
    }
    case ISD::SHL: {
      auto *CI = dyn_cast<ConstantSDNode>(RHSVal);
      assert(CI && "Cannot shift with non constant element");
      auto LHS = rec(LHSVal, false);
      // shift left of fractions
      return mul(LHS, { 1 << CI->getSExtValue(), 1 });
    }
    case ISD::SRL: {
      auto *CI = dyn_cast<ConstantSDNode>(RHSVal);
      assert(CI && "Cannot shift with non constant element");
      auto LHS = rec(LHSVal, false);
      //shift right of fractions
      return mul(LHS, { 1, 1 << CI->getSExtValue() });
    }
    case ISD::MUL: {
      if (auto *CI = dyn_cast<ConstantSDNode>(RHSVal))
        return mul(rec(LHSVal, false), { CI->getSExtValue(), 1 });

      if (auto *CI = dyn_cast<ConstantSDNode>(LHSVal))
        return mul({ CI->getSExtValue(), 1 }, rec(RHSVal, false));

      // 'isBase' should prevent this to happen
      llvm_unreachable("multiplication is supported only between"
                       "constant and ISD::ELEMENT_COUNT");
    }
    default: {
#ifndef NDEBUG
      S.getNode()->dumpr();
#endif
      llvm_unreachable("this node should be handled as a base");
    }
    }

    llvm_unreachable("'return' should not appear after the "
                     "'switch(Opcode)' statement ");
  };

  // Compute offset and base components
  fraction Offset = rec(N, false);
  DEBUG_WITH_TYPE("am_indexed_sve",
                  dbgs() << "Found fraction: "<< Offset.first << " / "
                  << Offset.second << "\n");
  const bool ECFound = !FoundEC.empty();
  bool BaseFound = !BaseComponent.empty();

  // Generate final Base and OffImm
  const SDLoc dl(N);
  if (BaseFound && ECFound && ((Offset.first % Offset.second) == 0)) {
    const int64_t IntegerOffset = Offset.first / Offset.second;
    if ((IntegerOffset <= Max) && (IntegerOffset >= Min)) {
      SDValue Tmp(BaseComponent[0].first, 0);

      if (BaseComponent[0].second) {
        SDValue Zero = CurDAG->getCopyFromReg(CurDAG->getEntryNode(), dl,
                                              AArch64::XZR, MVT::i64);
        MachineSDNode *Node =
            CurDAG->getMachineNode(AArch64::SUBXrr, dl, MVT::i64, Zero, Tmp);
        Tmp = SDValue(Node, 0);
      }

      // Build the base expression using the remaining components.
      for (unsigned i = 1; i < BaseComponent.size(); ++i) {
        SDValue RHS = SDValue(BaseComponent[i].first, 0);
        const unsigned Opcode =
            (BaseComponent[i].second) ? AArch64::SUBXrr : AArch64::ADDXrr;
        MachineSDNode *Node =
            CurDAG->getMachineNode(Opcode, dl, MVT::i64, Tmp, RHS);
        Tmp = SDValue(Node, 0);
      }
      Base = Tmp;
      OffImm = CurDAG->getTargetConstant(IntegerOffset, dl, MVT::i64);

      DEBUG_WITH_TYPE("am_indexed_sve",
            dbgs() << "\n*** SelectAddrModeIndexedSVE ***\n";
            dbgs() << "Machine function: "
                   << CurDAG->getMachineFunction().getName() << "\n";
            dbgs() << "Computing BASE and OFFSET for node:\n"; N.dumpr();
  ...
  DEBUG_WITH_TYPE("am_indexed_sve",
        dbgs() << "\n*** SelectAddrModeIndexedSVE ***\n";
        dbgs() << "Machine function: " << CurDAG->getMachineFunction().getName()
               << "\n";
        dbgs() << "Cannot decompose node:\n"; N.dumpr());

  // Something went wrong, we cannot select reg+immediate
  return false;
}
...
bool AArch64DAGToDAGISel::SelectSVERegRegAddrMode(SDValue N, unsigned Scale,
  ...
  DEBUG_WITH_TYPE("am_regreg_sve",
                  dbgs() << "*** SelectSVERegRegAddrMode<Scale = "
                  << Scale << ">\n";
                  N.dumpr(););
  // Ain't work with nodes other than ADD.
  if (Opcode != ISD::ADD) {
    return false;
  }
  ...
  // operation.  If yes, do not try to fold this node into the address
  // computation, since the computation will be kept.
  // TODO: reanable when masked_load/store names within the instrinfo.td will
  // become MemSDNodes
  // const SDNode *Node = N.getNode();
  // for (SDNode *UI : Node->uses()) {
  //   if (!isa<MemSDNode>(*UI))
  ...
  if (!IsRegisterWorthFolding) {
    DEBUG_WITH_TYPE("am_regreg_sve",
                    dbgs()
                    << "*** SelectSVERegRegAddrMode: not worth folding\n");
    return false;
  }
