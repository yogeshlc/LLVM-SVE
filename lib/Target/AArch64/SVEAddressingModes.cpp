...
private:
  ...
  bool onlyUsedForAddrModes(PHINode*, Instruction*, VectorType*, ConstantInt*);
  PHINode* findBetterPHI(PHINode*, BasicBlock*, BasicBlock*, VectorType*);
};
}
...
PHINode* SVEAddressingModes::findBetterPHI(PHINode *OrigPHI,
  ...
  for (Instruction &I : *OrigPHI->getParent()) {
    ...
    if (!match(PHI->getIncomingValue(Idx),
               m_Add(m_Specific(PHI), m_ElementCount(m_Value(EC)))))
      continue;

    VectorType *ECTy = dyn_cast<VectorType>(EC->getType());
    if (!ECTy || !ECTy->isScalable())
      continue;

    if (ECTy->getNumElements() != ElementCountTy->getNumElements())
      continue;
...
//                      ^^                       ^^^^^^
//                     sizeof(double)/sizeof(i8) == Scale
//  %wide.masked.load =
//      call <n x 2 x double> @llvm.masked.load(<n x 2 x double>* %uglygep2, ..)
//      :
//  %lsr.iv.next = add i64 %lsr.iv,
//                     mul (i64 elementcount (<n x 2 x i64> undef), i64 8)
bool SVEAddressingModes::onlyUsedForAddrModes(PHINode *PHI,
  ...
  while (!WorkList.empty()) {
    ...
    if (auto *BC = dyn_cast<BitCastInst>(U)) {
      ...
      int64_t IVScale = Scale->getSExtValue() * ECType->getNumElements();
...
static bool isScaledElementCountIVUpdate(
  ...
  // e.g.
  //  %update = add %phi, mul (i64 elementcount <n x 4 x i32>, i64 8)
  if (match(I, m_Add(m_PHI(PHI),
                     m_Mul(m_ElementCount(m_Value(EC)),
                           m_ConstantInt(Scale))))) {
    if (!Scale->getValue().isPowerOf2())
      return false;

    ElementCountTy = dyn_cast<VectorType>(EC->getType());
    if (!ElementCountTy || !ElementCountTy->isScalable())
      return false;
  ...
  //  %update =
  //    getelementptr i1, i1* %bc, mul (i64 elementcount <n x 4 x i32>, i64 8)
  //  %bc2 = bitcast i1* %update to i32*
  if (auto *Gep = dyn_cast<GetElementPtrInst>(I)) {
    Value *Opnd = Gep->getOperand(Gep->getNumIndices());
    bool Match1 = match(Opnd, m_ElementCount(m_Value(EC)));
    bool MatchN = match(Opnd, m_Mul(m_ElementCount(m_Value(EC)),
                                    m_ConstantInt(Scale)));

    // Gep update can either be scaled or unscaled
    if (!Match1 && !MatchN)
      return false;

    if (Match1)
      Scale = cast<ConstantInt>(ConstantInt::get(Opnd->getType(), 1, true));
...
bool SVEAddressingModes::OptimizeBlock(BasicBlock *BB) {
  ...
  for (BasicBlock::iterator it = BB->begin(), e = BB->end(); it != e;) {
    ...
    if (!isScaledElementCountIVUpdate(I, PHINodes, Scale, ElementCountTy,
                                      EntryBB, BackEdgeBB))
      continue;

    for (auto *PHI : PHINodes) {
      // Try to find an unscaled PHI with same elementcount.
      auto *OtherPHI = findBetterPHI(PHI, EntryBB, BackEdgeBB, ElementCountTy);
      ...
      for (auto *U : PHI->users()) {
        ...
        for (auto *UU : U->users()) {
          ...
          auto IsStructTy = [] (Type *Ty) { return Ty->isStructTy(); };
          if (std::any_of(gep_type_begin(Gep), gep_type_end(Gep), IsStructTy))
            continue;
      ...
      if (!Start->getType()->isPointerTy()) {
        ...
        if (CI && CI->isNullValue()) {
          ...
          if (!onlyUsedForAddrModes(PHI, I, ElementCountTy, Scale))
            continue;

          // Re-scale the start value
          Builder.SetInsertPoint(EntryBB->getTerminator());
          unsigned LogBase = Scale->getValue().logBase2();
          auto *ShiftAmount =
      ...
      if (Start->getType()->isPointerTy()) {
        ...
        int64_t ScaleVal = Scale->getSExtValue();
        ...
        // Rescale
        Scale = ConstantInt::get(Scale->getType(), ScaleVal / ElemTypeSize, true);

        // Sign extend scale and multiply with scalar indvar
        auto *Cast = Builder.CreateSExtOrBitCast(Scale, OtherPHI->getType());
        auto *Idx = Builder.CreateMul(OtherPHI, Cast);
