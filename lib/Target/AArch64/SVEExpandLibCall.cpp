...
struct SVEExpandLibCall : public FunctionPass {
  ...
  SVEExpandLibCall() : FunctionPass(ID) {
...
private:
  ...
  bool ExpandStructLoad(LoadInst *II);
  bool ExpandStructStore(StoreInst *II);
...
namespace llvm {
FunctionPass *createSVEExpandLibCallPass() {return new SVEExpandLibCall();}
}
...
template <class T>
static bool isStructMemOp(Instruction *I) {
  auto MemI = dyn_cast<T>(I);
  if (!MemI || !MemI->isSimple())
    return false;

  auto *Ptr = MemI->getPointerOperand();
  auto *STy = dyn_cast<StructType>(Ptr->getType()->getPointerElementType());

  if (!STy || (STy->getNumElements() == 0))
    return false;

  auto ElemTy = dyn_cast<VectorType>(STy->getElementType(0));
  if (!ElemTy || !ElemTy->isScalable())
    return false;

  return std::equal(STy->element_begin(), STy->element_end(),
                    STy->element_begin());
}
...
bool SVEExpandLibCall::ExpandMemCallToLoop(MemIntrinsic *II) {
  ...
  Value *Continue = Builder.CreateTest(TestInst::FIRST_TRUE, NextPred);
...
bool SVEExpandLibCall::ExpandCallToLoop(IntrinsicInst *II) {
  ...
  bool IsPredicated = isMaskedVectorIntrinsic(II->getIntrinsicID());
  ...
  Value *Predicate = IsPredicated ? II->getOperand(0) : PTrue;
  ...
  unsigned ArgIdx = IsPredicated ? 1 : 0;
  // Emit code to find the the scalar argument values from original vectors.
  for (; ArgIdx < II->getNumArgOperands(); ++ArgIdx) {
  ...
  auto Test = Builder.CreateTest(TestInst::ANY_TRUE, GPNext);
...
bool SVEExpandLibCall::ExpandStructLoad(LoadInst *II) {
  Value *Val = UndefValue::get(II->getType());
  auto *Ptr = II->getPointerOperand();
  auto *STy = dyn_cast<StructType>(Val->getType());

  IRBuilder<> Builder(&*II);
  Ptr = Builder.CreateBitCast(Ptr, STy->getElementType(0)->getPointerTo());

  for (unsigned i = 0; i < STy->getNumElements(); ++i) {
    auto *Idx = ConstantInt::get(Builder.getInt64Ty(), i);
    auto *Addr = Builder.CreateGEP(Ptr, Idx);

    auto Data = Builder.CreateLoad(Addr);
    Data->setDebugLoc(II->getDebugLoc());
    Val = Builder.CreateInsertValue(Val, Data, i);
  }
...
bool SVEExpandLibCall::ExpandStructStore(StoreInst *II) {
  auto *Val = II->getValueOperand();
  auto *Ptr = II->getPointerOperand();
  auto *STy = dyn_cast<StructType>(Val->getType());
  ...
  for (unsigned i = 0; i < STy->getNumElements(); ++i) {
    auto *Idx = ConstantInt::get(Builder.getInt64Ty(), i);
    auto *Addr = Builder.CreateGEP(Ptr, Idx);
    auto *Data = Builder.CreateExtractValue(Val, i);

    auto NewStore = Builder.CreateStore(Data, Addr);
    NewStore->setDebugLoc(II->getDebugLoc());
  }
...
bool SVEExpandLibCall::runOnFunction(Function &F) {
  ...
  SmallVector<Instruction *, 4> StructLoadWorkList;
  SmallVector<Instruction *, 4> StructStoreWorkList;
  ...
  for (auto I = inst_begin(F), E = inst_end(F); I != E; ++I) {
    ...
    else if (isStructMemOp<LoadInst>(&*I))
      StructLoadWorkList.push_back(&*I);
    else if (isStructMemOp<StoreInst>(&*I))
      StructStoreWorkList.push_back(&*I);
  }
  ...
  for (auto I : StructLoadWorkList)
    Changed |= ExpandStructLoad(cast<LoadInst>(I));

  for (auto I : StructStoreWorkList)
    Changed |= ExpandStructStore(cast<StoreInst>(I));
