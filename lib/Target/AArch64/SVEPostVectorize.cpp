...
bool SVEPostVectorize::runOnLoop(Loop *L) {
  ...
  while (!Worklist.empty()) {
    ...
    LoopOptimized |= Optimize_PostWhileConversion(BB);
...
bool SVEPostVectorize::Optimize_PreWhileConversion(BasicBlock *BB) {
  ...
  for (Instruction &I : *BB) {
    if (isa<PropFFInst>(I) && I.getType()->isVectorTy()) {
      ...
      // NOTE: Stride must be scaled when applied to pointers.
  ...
  for (BasicBlock::iterator it = BB->begin(), e = BB->end(); it != e;) {
    ...
    if (match(I, m_PropFF(m_PHI(PredPHI),
      ...
      for (unsigned i = 0, e = PredPHI->getNumIncomingValues(); i != e; ++i) {
        ...
        if (match(PHIOp, m_WhileLO(m_Specific(IndexFirst), m_Specific(Last))))
          continue;
