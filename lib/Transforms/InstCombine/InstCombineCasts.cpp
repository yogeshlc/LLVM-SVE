...
Instruction *InstCombiner::visitTrunc(TruncInst &CI) {
  ...
  // Transform trunc(seriesvector(X, Y)) to seriesvector(trunc X, trunc X)
  if (Src->hasOneUse() &&
      match(Src, m_SeriesVector(m_Value(A), m_ConstantInt(Cst)))) {
    Type *Ty = CI.getType()->getVectorElementType();
    Value *NewTrunc = Builder->CreateTrunc(A, Ty, A->getName()+".tr");
    return new SeriesVectorInst(CI.getType(), NewTrunc,
                                ConstantExpr::getTrunc(Cst, Ty));
  }
