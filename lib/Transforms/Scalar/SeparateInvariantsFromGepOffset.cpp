...
#define IF_LIV_RETURN_NULL_INSTEAD(Op, V, Res) { \
  if (!Op) { \
    Res.pop_back(); \
    Res.push_back(V); \
    return nullptr; \
...
static Value *OptimizeGEPExpr(Value *V, IRBuilder<> *Builder,
  ...
  if (match(V, m_CombineOr(m_SExt(m_Value(Op)), m_ZExt(m_Value(Op))))) {
    ...
    if (match(V, m_SExt(m_NSWAdd(m_Value(), m_Value()))) ||
      ...
      IF_LIV_RETURN_NULL_INSTEAD(Op, V, Res);
  ...
  if (match(V, m_Add(m_Value(LHS), m_Value(RHS)))) {
    ...
    // Move non-null to LHS
    if (!LHS)
      std::swap(LHS, RHS);

    // Result is loop invariant, this node is also loop invariant
    IF_LIV_RETURN_NULL_INSTEAD(LHS, V, Res);

    // LHS is not loop-invariant
    return LHS;
  }
...
#undef IF_LIV_RETURN_NULL_INSTEAD
