...
struct LoopSpeculativeBoundsCheck : public LoopPass {
  ...
  void collectStridedAccess(Loop *, Value *);
  bool instructionSafeForVersioning(Loop *, Instruction *);
  void addPHINodes(Loop *, Loop *);
  const char *getPassName() const override { return "Loop SBC"; }
};
}

/// \brief Collects stride access from a given value.
void LoopSpeculativeBoundsCheck::collectStridedAccess(Loop *L,
                                                      Value *MemAccess) {
  Value *Ptr = nullptr;
  ...
  Value *Stride = getStrideFromPointer(Ptr, SE, L);
  if (!Stride)
    return;
...
bool LoopSpeculativeBoundsCheck::processLoop(Loop *L) {
  ...
  if (isSuitableLoop(L, AST)) {
    ...
    LAI = &LAA->getInfo(SpeculativeLoop, Strides);
