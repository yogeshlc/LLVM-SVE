; RUN: opt -S < %s -basicaa -loop-vectorize -force-scalable-vectorization -force-vector-predication -O3 -force-vector-interleave=2 2>&1 | FileCheck %s
...
; CHECK: %predicate.entry = propff <n x 4 x i1> shufflevector
; CHECK: %predicate.entry[[NEXT:[0-9]+]] = propff <n x 4 x i1> %predicate.entry,
...
; CHECK: %[[CALL:[0-9]+]] = tail call float @llvm.aarch64.sve.adda.nxv4f32(
; CHECK-SAME: %predicate,
; CHECK-SAME: %vec.phi
; CHECK: %[[CALL2]] = tail call float @llvm.aarch64.sve.adda.nxv4f32(
; CHECK-SAME: %predicate[[PNEXT]],
; CHECK-SAME: %[[CALL]]
...
; CHECK-LABEL: min.iters.checked:
; CHECK: %predicate.entry = propff <n x 2 x i1> shufflevector
; CHECK: %predicate.entry[[NEXT:[0-9]+]] = propff <n x 2 x i1> %predicate.entry,
...
; CHECK: %[[CALL:[0-9]+]] = tail call double @llvm.aarch64.sve.adda.nxv2f64(
; CHECK-SAME: %predicate,
; CHECK-SAME: %vec.phi
; CHECK: %[[CALL2]] = tail call double @llvm.aarch64.sve.adda.nxv2f64(
; CHECK-SAME: %predicate[[PNEXT]],
; CHECK-SAME: %[[CALL]]
for.body:                                         ; preds = %for.body.preheader, %for.body
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body ], [ 0, %for.body.preheader ]
...
; CHECK-SAME: %[[PFIRST]]
; CHECK: %[[PSECOND:[0-9]+]] = and <n x 2 x i1> %{{.*}}, %predicate[[PNEXT]]
; CHECK: %wide.masked.load[[LDSECOND:[0-9]+]] = call <n x 2 x double> @llvm.masked.load.nxv2f64
; CHECK-SAME: %[[PSECOND]]
; CHECK: %[[PAND:[0-9]+]] = and <n x 2 x i1> %{{[0-9]+}}, %predicate
; CHECK: %[[CALL:[0-9]+]] = call double @llvm.aarch64.sve.adda.nxv2f64(
; CHECK-SAME: %[[PAND]],
; CHECK-SAME: %vec.phi
; CHECK: %wide.masked.load
; CHECK: %[[PANDNEXT:[0-9]+]] = and <n x 2 x i1> %{{[0-9]+}}, %predicate[[PNEXT]]
; CHECK: %[[CALL2]] = call double @llvm.aarch64.sve.adda.nxv2f64(
; CHECK-SAME: %[[PANDNEXT]],
; CHECK-SAME: %[[CALL]]
; CHECK: adda
entry:
