...
; ORDER: %wide.masked.load = tail call <n x 4 x i32> @llvm.masked.load.nxv4i32(<n x 4 x i32>* %2
; ORDER-DAG: [[C:%[0-9]+]] = getelementptr inbounds float, float* %c, i64 %index
; ORDER-DAG: [[C2:%[0-9]+]] = bitcast float* [[C]] to <n x 4 x float>*
; ORDER-DAG: %wide.masked.load{{[0-9]+}} = tail call <n x 4 x float> @llvm.masked.load.nxv4f32(<n x 4 x float>*
; ORDER: guarded.block
; ORDER: tail call void @llvm.masked.store.nxv4f32(<n x 4 x float> %{{[0-9]+}}, <n x 4 x float>* [[C2]]
; ORDER: tail call void @llvm.masked.store.nxv4f32(<n x 4 x float> %
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  call void @llvm.masked.store.nxv4f32(<n x 4 x float> %14, <n x 4 x float>* %15, i32 4, <n x 4 x i1> %10), !tbaa !5
  %index.next = add nuw nsw i64 %index, elementcount (<n x 4 x i64> undef)
  %16 = add nuw nsw i64 %index, elementcount (<n x 4 x i64> undef)
  %17 = seriesvector i64 %16, 1 as <n x 4 x i64>
  %18 = icmp ult <n x 4 x i64> %17, shufflevector (<n x 4 x i64> insertelement (<n x 4 x i64> undef, i64 1024, i32 0), <n x 4 x i64> undef, <n x 4 x i32> zeroinitializer)
  %predicate.next = propff <n x 4 x i1> %predicate, %18
  %19 = test first true <n x 4 x i1> %predicate.next
  br i1 %19, label %vector.body, label %for.cond.cleanup, !llvm.loop !7
