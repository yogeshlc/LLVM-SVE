; RUN: opt -mtriple aarch64-linux-generic -march=armv8-a -mattr=+sve -O3 -loop-vectorize -force-scalable-vectorization -enable-non-consecutive-stride-ind-vars -force-vector-width=4 -disable-loop-unrolling -S < %s | FileCheck %s
...
; CHECK-DAG: %[[BITCAST:[0-9]+]] = bitcast i8** %{{[0-9]+}} to <n x 4 x i64>*
; CHECK-DAG: %[[LOAD:[a-zA-Z0-9.]+]] = load <n x 4 x i64>, <n x 4 x i64>* %[[BITCAST]], align 8
...
for.body:                                         ; preds = %for.cond
; CHECK-DAG: %[[BITCAST:[0-9]+]] = bitcast %struct.foo** %{{[0-9]+}} to <n x 4 x i64>*
; CHECK-DAG: %[[LOAD:[a-zA-Z0-9.]+]] = load <n x 4 x i64>, <n x 4 x i64>* %[[BITCAST]], align 8
