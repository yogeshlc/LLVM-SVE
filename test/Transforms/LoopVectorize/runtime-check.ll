...
;CHECK-LABEL: define i32 @unhosisted_load
;CHECK: for.body.preheader:
;CHECK: br i1 %cmp.zero, label %scalar.ph, label %vector.memcheck, !dbg [[BODY_LOC:![0-9]+]]
;CHECK: vector.memcheck:
;CHECK: icmp uge float* %scaleidx, %a
;CHECK: icmp ule float* %scaleidx, %scevgep
;CHECK: br i1 %conflict.rdx, label %scalar.ph, label %vector.ph, !dbg [[BODY_LOC]]
;CHECK: load <4 x float>
define i32 @unhosisted_load(float* nocapture %a, float* nocapture %b, float* nocapture %c, i32 %n) nounwind uwtable ssp {
entry:
  %cmp6 = icmp sgt i32 %n, 0, !dbg !6
