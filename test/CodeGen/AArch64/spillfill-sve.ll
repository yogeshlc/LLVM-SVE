...
define void @spillfill_1p1v_1stackarg_sp(i64 %a1, i64 %a2, i64 %a3, i64 %a4,
                                         ...
                                         i64 %a9) {
; CHECK-LABEL: spillfill_1p1v_1stackarg_sp
; CHECK-DAG: addvl   sp, sp, #-[[SVESIZE:[0-9]+]]
  %i0 = alloca i64
  %v0 = alloca %vtype
  %p0 = alloca %ptype
...
; CHECK-DAG: addvl  [[BASEADDR_A9:x[0-9]+]], sp, #[[SVESIZE]]
; CHECK-DAG: ldr    [[LD_A9:x[0-9]+]], {{\[}}[[BASEADDR_A9]], #16]
; CHECK-DAG: str    [[LD_A9]], [sp,
  store i64 %a9, i64* %i0
...
define i8* @spillfill_1p1v_1stackarg_fp(i64 %a1, i64 %a2, i64 %a3, i64 %a4,
                                         i64 %a5, i64 %a6, i64 %a7, i64 %a8,
                                         i64 %a9) #0 {
; CHECK-LABEL: spillfill_1p1v_1stackarg_fp
; CHECK-DAG: addvl   sp, sp, #-[[SVESIZE:[0-9]+]]
  %i0 = alloca i64
  %v0 = alloca %vtype
  %p0 = alloca %ptype
...
; CHECK-DAG: addvl  [[BASEADDR_A9:x[0-9]+]], x29, #[[SVESIZE]]
; CHECK-DAG: ldr    [[LD_A9:x[0-9]+]], {{\[}}[[BASEADDR_A9]]]
; CHECK-DAG: str    [[LD_A9]], [sp,
  store i64 %a9, i64* %i0
...
; CHECK-DAG: st1h {[[VLD]].h}, {{p[0-9]+}}, [x0, #7, mul vl]
  %1 = load %vtype, %vtype* @vread
  store %vtype %1, %vtype* %v0
...
; CHECK-DAG:  ldr [[OFFSET:x9]], {{\[x[0-9]+}}, :lo12:iread]
; CHECK-DAG:  ld1h {[[ZVALUE:z[0-9]+]].h},
; CHECK-DAG:  cnth [[CNT:x[0-9]+]]
; CHECK-DAG:  mul  [[ELEM:x[0-9]+]], [[CNT]], [[OFFSET]]
; CHECK-DAG:  str  [[SPILL:x[0-9]+]], {{\[sp\]}}
; CHECK-DAG:  add  [[SPILL]], sp, #160
; CHECK-DAG:  add  [[BASE:x[0-9]+]], [[SPILL]], #0
; CHECK-DAG:  st1h {[[ZVALUE]].h}, {{p[0-9]+}}, {{\[}}[[BASE]], [[ELEM]], lsl #1]
...
; CHECK-LABEL: spillfill_1p1v2i_sp_fp_nosaves_call9params_callersave_pred
  ...
  %cond = test first true <n x 8 x i1> %pred
