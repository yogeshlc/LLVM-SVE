; RUN: llc %s -o - -enable-shrink-wrap=true -aarch64-sve-postvec=false  -disable-post-ra -disable-fp-elim | FileCheck %s --check-prefix=CHECK --check-prefix=ENABLE
; RUN: llc %s -o - -enable-shrink-wrap=false -aarch64-sve-postvec=false  -disable-post-ra -disable-fp-elim | FileCheck %s --check-prefix=CHECK --check-prefix=DISABLE
